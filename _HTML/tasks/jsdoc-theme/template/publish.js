...
var template = require('jsdoc/template'),
	...
	isHtmldoc = env.conf.htmldoc === true,
	isSASSdoc = env.conf.sassdoc === true,
	data,
	view,
	_docsData = {
		sourcefiles: {},
...
var isHtmldoc = navOptions.isHtmldoc;
var isSASSdoc = navOptions.isSASSdoc;

function _lang(en, ru) {
	if (navOptions.theme == "wezom-ru") {
		return ru;
	}
	return en;
}

function _htmldocLang(_langObg) {
	let sample = 'jsdoc';
		...
		if (symbols[module.longname]) {
			...
			.filter(function(symbol) {
				...
				.map(function(symbol) {
					...
					var _rel = symbol.moduleLocal ? './' : '';

					if ((symbol.kind === 'class' || symbol.kind === 'function') && _rel.length) {
						var _includeMtds = [
							'<small>require</small>',
							symbol.name.replace('module:', '<b>const</b> {moduleName} = require("'+_rel) + '")\%signature\%;'
						];
						if (symbol.moduleNoES6Import !== true) {
							_includeMtds = _includeMtds.concat([
								'<small>es6 import</small>',
								symbol.name.replace('module:', '<b>import</b> {moduleImportName} <b>from</b> "'+_rel) + '";',
								'<b>const</b> {moduleName} = {moduleImportName}\%signature\%;'
							]);
						}
						symbol._testProp = true;
						symbol.name = '<div class="moduleName">'+_includeMtds.join('<br>')+'</div>';
					}
	...
	if (members.namespaces.length) {
		...
		members.namespaces.forEach(function(n) {
			if (!hasOwnProp.call(seen, n.longname)) {
				let _longname = n.longname;
				if (isHtmldoc) {
					_longname = _longname.split('_');
					let _html = _longname.pop();
					_longname = `${_longname.join('_')}.${_html}`;
				}
				n._longname = _longname;
				nav.namespace.members.push(linkto(n.longname, n._longname.replace("module:", "")));
			}
...
exports.publish = function(taffyData, opts, tutorials) {
	...
	for (var longname in helper.longnameToUrl) {
		if (hasOwnProp.call(helper.longnameToUrl, longname)) {
			...
			if (myClasses.length) {
				generate('class', _lang('Class', 'Класс') + ': ' + myClasses[0].name, myClasses, helper.longnameToUrl[longname]);
			}
			...
			if (myModules.length) {
				generate('module', _lang('Module', 'Модуль') + ': ' + myModules[0].name, myModules, helper.longnameToUrl[longname]);
			}
			...
			if (myNamespaces.length) {
				if (isHtmldoc) {
					let _longname = myNamespaces[0].name.split('_');
					let _html = _longname.pop();
					myNamespaces[0].name = `${_longname.join('_')}.${_html}`;
				}
				generate('namespace', _htmldocLang({
					'jsdoc': ["Namespace", "Пространство имени"],
					'htmldoc': ["Page", "Страница"]
				}) +': ' + myNamespaces[0].name, myNamespaces, helper.longnameToUrl[longname]);
			}
			...
			if (myMixins.length) {
				generate('mixin',  _lang('Mixin', 'Миксин') + ': ' + myMixins[0].name, myMixins, helper.longnameToUrl[longname]);
			}
			...
			if (myInterfaces.length) {
				generate('interface', _lang('Interface', 'Интерфейс') + ': ' + myInterfaces[0].name, myInterfaces, helper.longnameToUrl[longname]);
			}
			...
			if (myExternals.length) {
				generate('external', _lang('External', 'Внешний') + ': ' + myExternals[0].name, myExternals, helper.longnameToUrl[longname]);
			}
		}
	}

	// TODO: move the tutorial functions to templateHelper.js
	function generateTutorial(title, tutorial, filename, sources) {
		var tutorialData = {
			title: title,
		...
		// if (tutorial.children.length) {
		// 	console.log(tutorial.children);
		// 	console.log(navigationMaster);
		// 	console.log('\n\n');
		// }
	...
	function saveChildren(node) {
		node.children.forEach(function(child) {
			generateTutorial(_lang('Tutorial', 'Туториал') + ': ' + child.title, child, helper.tutorialToUrl(child.name), _docsData);
			saveChildren(child);
		});
	}
