<?php
	...
	class User {
		...
		function __construct() {
            ...
            if($this->_info) {
                $__res = DB::select('catalog_id')->from('users_wishlist')->where('user_id', '=', $this->_info->id)->find_all();
                foreach($__res AS $__obj) {
                    $this->wishlist[] = $__obj->catalog_id;
                }

                $__res = DB::select('orders_items.price', 'orders_items.count', 'orders.currency')
                    ->from('orders_items')
                    ->join('orders')->on('orders.id', '=', 'orders_items.order_id')
                    ->where('orders.user_id', '=', $this->_info->id)
                    ->where('orders.status', '=', 1)
                    ->find_all();
                foreach($__res AS $__obj) {
                    $this->ordersAmount += Support::usd($__obj->price, $__obj->currency, true) * $__obj->count;
                }
                $__res = DB::select(array(DB::expr('MAX(percent)'), 'max'))
                    ->from('prog')
                    ->where('amount', '<=', $this->ordersAmount)
                    ->find();
                if($__res) {
                    $this->discount = $__res->max;
                }
            }
		}
        ...
        public static function wishlist() {
            return User::factory()->wishlist;
        }


        public static function ordersAmount() {
            return User::factory()->ordersAmount;
        }


        public static function discount() {
            return User::factory()->discount;
        }
