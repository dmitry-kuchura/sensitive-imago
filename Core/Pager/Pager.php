...
class Pager {
    ...
    public static function factory($current, $total, $per_page, $ajax = NULL) {
        return new Pager($current, $total, $per_page, $ajax);
    }

    public function __construct($current, $total, $per_page, $ajax) {
        if (!$per_page) {
            return NULL;
        }
        try {
