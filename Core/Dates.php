...
class Dates {
    ...
    const HOUR = 3600;
    const MINUTE = 60;
    // Available formats for Dates::months()
    const MONTHS_LONG = '%B';
     ...
     *
     *     $seconds = Dates::offset('America/Chicago', 'GMT');
     *
     ...
     *
     *     $seconds = Dates::seconds(); // 01, 02, 03, ..., 58, 59, 60
     *
     ...
     *
     *     $hours = Dates::hours(); // 01, 02, 03, ..., 10, 11, 12
     *
     ...
     *
     *     $type = Dates::ampm(12); // PM
     *     $type = Dates::ampm(1);  // AM
     *
     ...
     *
     *     Dates::days(4, 2010); // 1, 2, 3, ..., 28, 29, 30
     *
     ...
     *
     *     $years = Dates::years(2000, 2010); // 2000, 2001, ..., 2009, 2010
     *
     ...
     * Also consider using [Dates::fuzzy_span] when displaying a span.
     *
     *     $span = Dates::span(60, 182, 'minutes,seconds'); // array('minutes' => 2, 'seconds' => 2)
     *     $span = Dates::span(60, 182, 'minutes'); // 2
     *
    ...
    public static function span($remote, $local = NULL, $output = 'years,months,weeks,days,hours,minutes,seconds') {
        ...
        if (isset($output['years'])) {
            $timespan -= Dates::YEAR * ($output['years'] = (int) floor($timespan / Dates::YEAR));
        }

        if (isset($output['months'])) {
            $timespan -= Dates::MONTH * ($output['months'] = (int) floor($timespan / Dates::MONTH));
        }

        if (isset($output['weeks'])) {
            $timespan -= Dates::WEEK * ($output['weeks'] = (int) floor($timespan / Dates::WEEK));
        }

        if (isset($output['days'])) {
            $timespan -= Dates::DAY * ($output['days'] = (int) floor($timespan / Dates::DAY));
        }

        if (isset($output['hours'])) {
            $timespan -= Dates::HOUR * ($output['hours'] = (int) floor($timespan / Dates::HOUR));
        }

        if (isset($output['minutes'])) {
            $timespan -= Dates::MINUTE * ($output['minutes'] = (int) floor($timespan / Dates::MINUTE));
        }
     ...
     *
     *     $span = Dates::fuzzy_span(time() - 10); // "moments ago"
     *     $span = Dates::fuzzy_span(time() + 20); // "in moments"
     *
    ...
    public static function fuzzy_span($timestamp, $local_timestamp = NULL) {
        ...
        if ($offset <= Dates::MINUTE) {
            $span = 'moments';
        } elseif ($offset < (Dates::MINUTE * 20)) {
            $span = 'a few minutes';
        } elseif ($offset < Dates::HOUR) {
            $span = 'less than an hour';
        } elseif ($offset < (Dates::HOUR * 4)) {
            $span = 'a couple of hours';
        } elseif ($offset < Dates::DAY) {
            $span = 'less than a day';
        } elseif ($offset < (Dates::DAY * 2)) {
            $span = 'about a day';
        } elseif ($offset < (Dates::DAY * 4)) {
            $span = 'a couple of days';
        } elseif ($offset < Dates::WEEK) {
            $span = 'less than a week';
        } elseif ($offset < (Dates::WEEK * 2)) {
            $span = 'about a week';
        } elseif ($offset < Dates::MONTH) {
            $span = 'less than a month';
        } elseif ($offset < (Dates::MONTH * 2)) {
            $span = 'about a month';
        } elseif ($offset < (Dates::MONTH * 4)) {
            $span = 'a couple of months';
        } elseif ($offset < Dates::YEAR) {
            $span = 'less than a year';
        } elseif ($offset < (Dates::YEAR * 2)) {
            $span = 'about a year';
        } elseif ($offset < (Dates::YEAR * 4)) {
            $span = 'a couple of years';
        } elseif ($offset < (Dates::YEAR * 8)) {
            $span = 'a few years';
        } elseif ($offset < (Dates::YEAR * 12)) {
            $span = 'about a decade';
        } elseif ($offset < (Dates::YEAR * 24)) {
            $span = 'a couple of decades';
        } elseif ($offset < (Dates::YEAR * 64)) {
     ...
     * Converting the other direction is done using {@link Dates::dos2unix}.
     *
     *     $dos = Dates::unix2dos($unix);
     *
     ...
     * this is needed, but some binary formats use it (eg: zip files.)
     * Converting the other direction is done using {@link Dates::unix2dos}.
     *
     *     $unix = Dates::dos2unix($dos);
     *
     ...
     *
     *     $time = Dates::formatted_time('5 minutes ago');
     *
    ...
    public static function formatted_time($datetime_str = 'now', $timestamp_format = NULL, $timezone = NULL) {
        $timestamp_format = ($timestamp_format == NULL) ? Dates::$timestamp_format : $timestamp_format;
        $timezone = ($timezone === NULL) ? Dates::$timezone : $timezone;
    ...
    /**
     *  Get hours or minutes from seconds
     *  @param  int    $seconds - seconds we want to translate
     *  @param  string $type    - 'hrs': hours, else string - minutes
     *  @return int             - count of hours or minutes
     */
    public static function getTime($seconds, $type = 'min') {
        $min = ceil($seconds / 60);
        $hrs = floor($min / 60);
        if ($type == 'hrs') {
            return $hrs;
    ...
    /**
     *  Get weekday short name from its number 0-6
     *  @param  mixed  $number - number of the week day. Notice: 1 and 01 - the same things
     *  @return string         - short name of the week day
     */
