    ...
    class Route {
        ...
        protected function setLanguage() {
            if ( class_exists('I18n') ) {
                ...
                $result = DB::select()->from('i18n')->execute()->as_array();
                $list = array();
                $uri = explode('/', $this->_uri);
                foreach ($result as $lang) {
                    $list[$lang['alias']] = $lang;
                    if ($lang['default']) {
                ...
                if ( !$uri[1] ) {
                    ...
                    if (isset($list[$this->_params['lang']])) {
                        \I18n::lang($this->_params['lang']);
                    } else {
                        \I18n::lang(\I18n::$default_lang);
                    }
                }
                Config::set('languages', $list);
            } else {
        ...
        protected function setURI() {
            if(!empty($_SERVER['REQUEST_URI'])) {
                ...
                $this->_uri = htmlspecialchars($tmp[0]);
                $get = array();
                foreach($_GET AS $key => $value) {
                    $get[] = $key.'='.$value;
                }
                $_SERVER['REQUEST_URI'] = '/'.$this->_uri.($get ? '?'.implode('&', $get) : NULL);
                return $this->_uri;
            }
        ...
        protected function setGET( $get ) {
            ...
            foreach ($get as $element) {
                $g = explode('=', $element);
                $_GET[Arr::get($g, 0)] = htmlspecialchars(Arr::get($g, 1));
            }
        }
        ...
        protected function start($path, $action) {
            if(isset($_GET[$this->_controller.'/'.$this->_action])) {
                unset($_GET[$this->_controller.'/'.$this->_action]);
            }
