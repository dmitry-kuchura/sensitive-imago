...
use Core\View;
use Core\QB\DB;
use Wezom\Modules\Config\Models\Phones AS Model;

class Phones extends \Wezom\Modules\Base {

    public $tpl_folder = 'Config/Phones';

    function before() {
        parent::before();
        $this->_seo['h1'] = __('Номера телефонов');
        $this->_seo['title'] = __('Номера телефонов');
        $this->setBreadcrumbs(__('Номера телефонов'), 'wezom/' . Route::controller() . '/index');
    }
    ...
    function editAction() {
        if ($_POST) {
            ...
            $check = DB::select()->from('contacts_phones')->where('lang', 'LIKE', $post['lang'])->where('group', '=', $post['group'])->find_all();
            if (count($check) > 0) {
                Message::GetMessage(0, __('Для каждого языка в шапку добавляется по одному телефону!'));
            } else {
                if (Model::valid($post)) {
