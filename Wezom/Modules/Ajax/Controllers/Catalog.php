<?php
    ...
    class Catalog extends \Wezom\Modules\Ajax {
        ...
        public function getItemAction() {
            ...
            $item = DB::select('catalog.id', 'catalog_i18n.name', 'catalog.price', 'catalog.image', array('brands.id', 'brand_id'), array('brands_i18n.name', 'brand_name'))
                ->from('catalog')
                ...
                ->join('brands_i18n', 'LEFT')->on('brands_i18n.row_id', '=', 'brands.id')->on('brands_i18n.language', '=', DB::expr('"'.\I18n::$default_lang.'"'))
                ->where('catalog.id', '=', $id)
                ->find();

            $__res = DB::select('sizes.alias', 'sizes.name')
                ->from('catalog_sizes')
                ->join('sizes')->on('catalog_sizes.size_alias', '=', 'sizes.alias')
                ->where('catalog_sizes.catalog_id', '=', $id)
                ->order_by('sizes.sort', 'ASC')
                ->find_all();
            $iSizes = array();
            ...
            $__res = DB::select('colors.alias', 'colors_i18n.name')
                ->from('catalog_colors')
                ->join('colors')->on('catalog_colors.color_alias', '=', 'colors.alias')
                ->join('colors_i18n')->on('colors_i18n.row_id', '=', 'colors.id')
                ->where('catalog_colors.catalog_id', '=', $id)
                ->where('colors_i18n.language', '=', \I18n::$default_lang)
                ->order_by('colors.sort', 'ASC')
                ->find_all();
            $iColors = array();
            foreach($__res AS $value) {
                $iColors[] = array(
            ...
            die(json_encode(array(
                ...
                'item' => array(
                    ...
                    'image' => is_file(HOST.HTML::media('images/catalog/small/'.$item->image)) ? HTML::media('images/catalog/small/'.$item->image) : NULL,
                    'image_big' => is_file(HOST.HTML::media('images/catalog/big/'.$item->image)) ? HTML::media('images/catalog/big/'.$item->image) : NULL,
                ),
                'sizes' => $iSizes,
                'colors' => $iColors,
            )));
        ...
        public function addItemToRelatedAction() {
            ...
            $row = DB::select('id')
                ->from('catalog_related')
                ->where('who_id', '=', $with_id)
                ->where('with_id', '=', $who_id)
                ->find();
            if(!$row) {
                Common::factory('catalog_related')->insert(array(
                    'who_id' => $with_id,
