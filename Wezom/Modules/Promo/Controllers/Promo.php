<?php
    ...
    use Wezom\Modules\Promo\Models\Promo AS Model;

    class Promo extends \Wezom\Modules\Base {

        public $tpl_folder = 'Promo';
        public $page;
        public $limit;
        public $offset;
        ...
        function before() {
            parent::before();
            $this->_seo['h1'] = __('Купоны на скидку');
            $this->_seo['title'] = __('Купоны на скидку');
            $this->setBreadcrumbs(__('Купоны на скидку'), 'wezom/'.Route::controller().'/index');
        ...
        function editAction () {
            if ($_POST) {
                ...
                if( Model::valid($post) ) {
                    ...
                    if($res) {
                        DB::delete('promo_groups')->where('promo_id', '=', Route::param('id'))->execute();
                        foreach (Arr::get($_POST, 'GROUPS', array()) as $gr) {
                            Common::factory('promo_groups')->insert(array(
                                'group_id' => $gr,
                                'promo_id' => Route::param('id'),
                            ));
                        }
            ...
            $promoGroups = array();
            $res = DB::select('group_id')->from('promo_groups')->where('promo_id', '=', Route::param('id'))->find_all();
            foreach ($res as $obj) {
