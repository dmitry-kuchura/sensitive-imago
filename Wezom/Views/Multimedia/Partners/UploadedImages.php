<?php if (count($images)): ?>
    <?php foreach($images as $im): ?>
        <?php if (is_file(HOST.Core\HTML::media('images/partners_images/main/'.$im->image))): ?>
            <div class="loadedBlock <?= $im->status == 1 ? 'chk' : ''; ?>" data-image="<?=$im->id; ?>">
                <div class="loadedImage">
                    <img src="<?php echo Core\HTML::media('images/partners_images/main/'.$im->image); ?>" />
                </div>
                <div class="loadedControl">
                    ...
                    <div class="loadedCtrl loadedView">
                        <button class="btn btn-primary btnImage" alt="Просмотр" href="<?php echo Core\HTML::media('images/partners_images/original/'.$im->image); ?>"><i class="fa-zoom-in"></i></button>
                    </div>
                    <?php if(\Core\User::god() || \Core\User::get_access_for_controller('gallery') == 'edit'): ?>
                        <div class="loadedCtrl">
                            <button class="btn btn-warning" alt="Редактировать" href="<?php echo \Core\General::crop('partners_images', 'main', $im->image, $_SERVER['HTTP_REFERER']); ?>"><i class="fa-pencil"></i></button>
                        </div>
