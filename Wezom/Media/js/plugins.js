...
$(document).ready(function(){
	...
	var sumRecount = function(spinEl,val){
		...
		var spinCost = $('.tableOrderItemsCost input',spinTr);
		var spinSumm = $('.tableOrderItemsSumm',spinTr);
		
		spinSumm.text((parseFloat(spinCost.val()) * parseInt(val)).toFixed(2));
	};
	$('.tableOrderItemsCost input').on('keyup', function(){
        var spinner = $(this).closest('tr').find('.spinner');
		sumRecount(spinner,spinner.val());
	});
