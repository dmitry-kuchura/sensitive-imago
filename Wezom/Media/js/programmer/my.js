...
$(document).ready(function(){
    ...
    var __lang;
    if($('.qe').length){
        var __filename = $('#translateFilename').data('filename');
        $('.qe').liQuickEdit({
            qeOpen: function (el, text) {},
            qeClose: function (el, text) {
                __key = el.closest('tr').find('td:first-child').text();
                __lang = el.data('lang');
                $.ajax({
                    url: '/wezom/ajax/saveTranslation',
                    type: 'POST',
                    dataType: 'JSON',
                    data: {
                        key: __key,
    ...
    if($('.qe2').length){
        $('.qe2').liQuickEdit({
            ...
            qeClose: function (el, text) {
                ...
                __lang = el.data('lang');
                $.ajax({
                    url: '/wezom/ajax/saveTranslationBackend',
                    type: 'POST',
                    dataType: 'JSON',
                    data: {
                        key: __key,
    ...
    $('body').on('click', '.translitAction', function(){
        ...
        if (source.length === 0) {
            generate('Не заполнено название на языке по умолчанию! Язык по-умолчанию - ('+ lang +')', 'warning');
        }
        $.ajax({
    ...
    if ( $('#orderParameters').length ) {
        ...
        var setAmount = function(){
            ...
            $('#orderItemsList tr').each(function(){
                var cost = parseFloat( $(this).find('.tableOrderItemsCost input').val() );
        ...
        $('#orderItems').on('click', function(){
            ...
            table.find('tr').each(function(){
                ...
                var size_alias = it.find('.size_alias').val();
                var color_alias = it.find('.color_alias').val();
                var price = it.find('.price').val();
                $.ajax({
...
$(function(){
    ...
    var search = function() {
        ...
        var url = '/wezom/ajax/catalog/searchItems';
        if($('#thisIsNewsPage').length) {
            url = '/wezom/ajax/news/searchItems';
        }
            ...
            success: function (resp) {
                if(!parseInt(resp.count)) {
                    ...
                    for (var i = 0; i < resp.items.length; i++) {
                        ...
                        html += '<div class="relatedName">' + resp.items[i].name + '<br>' + resp.items[i].cost + ' $</div>';
                        html += '</div>';
                    }
                    html += pager;
                    list.html(html);
    ...
    list.on('click', '.relatedItem', function(){
        ...
        var url = '/wezom/ajax/catalog/addItemToRelated';
        if($('#thisIsNewsPage').length) {
            url = '/wezom/ajax/news/addItemToRelated';
        }
    ...
    $('.listSimilar').on('click', '.relatedItem', function(){
        ...
        var url = '/wezom/ajax/catalog/removeItemFromRelated';
        if($('#thisIsNewsPage').length) {
            url = '/wezom/ajax/news/removeItemFromRelated';
        }
            ...
            $.ajax({
                ...
                success: function(resp){
                    if(resp.success) {
                        ...
                        if(resp.item.cost) {
                            html += '<div class="someBlock"><b>Цена:</b> ' + resp.item.cost + ' $</div>';
                        }
                        if(resp.item.uid) {
                        ...
                        if(resp.sizes) {
                            var s = '';
                            for(var i = 0; i < resp.sizes.length; i++) {
                                s += '<option value="' + resp.sizes[i].alias + '">' + resp.sizes[i].name + '</option>';
                            }
                            $('#f_sizes').html(s);
                        }
                        if(resp.sizes) {
                            var c = '';
                            for(var j = 0; j < resp.colors.length; j++) {
                                c += '<option value="' + resp.colors[j].alias + '">' + resp.colors[j].name + '</option>';
                            }
                            $('#f_colors').html(c);
                        }
                    }
...
$(function(){
    ...
    // E-Mail to users that asked for returns
    $('.askForReturns').on('click', function(){
        preloader();
        var id = $(this).data('id');
        $.ajax({
            url: '/wezom/ajax/askForReturns',
            type: 'POST',
            dataType: 'JSON',
            data: {
                id: id
            ...
            success: function(data){
                if(data.success) {
                    generate(data.response, 'success', 5000);
                } else {
                    generate(data.response, 'warning');
                }
                preloader();
