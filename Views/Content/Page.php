<?php if (count($kids)): ?>
    <div id="slider_else_universal" class="slider_else_universal">
        <?php foreach ($kids as $kid): ?>
            <?php
            if (is_file(HOST . Core\HTML::media('images/content/main/' . $kid->image))) {
                $image = Core\HTML::media('images/content/main/' . $kid->image);
            } else {
                $image = Core\HTML::media('pic/no-image.png');
            }
            ?>
            <div class="items_universal">
                <a href="<?php echo Core\HTML::link($kid->alias); ?>" class="advantage advantage--mini">
